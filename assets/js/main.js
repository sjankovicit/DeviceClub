...
function ajaxCategories(callbackSuccess, error){
    $.ajax({
...
function showCategories(){
    ...
    const fnOnSuccess = function(categories){
        printCategories(categories);
    }

    ajaxCategories(fnOnSuccess, fnOnError);
}
...
function filterByCategory(movie, catId){
    return filtered = movie.filter(x => x.categories.id == catId);
}
...
function bindCartEvents() {
    ...
    $(".add-to-cart").click(priceFn);
}

function checkPrice(){
    let price = priceInCart();
...
function generatePrice(price){
    ...
    for(let p of price){
        finalPrice += `${p.price.new * p.quantity}`;
    }
...
function priceAlreadyInCart(){
        return price.filter(p => p.id == id).length;
    }

function addFirstPrice(){
    let price = [];
        price[0] = {
            id : id,
            quantity : 1
...
function addPriceToLocalStorage() {
    let price = moviesInCart();
    price.push({
        id : id,
        quantity : 1
    });
